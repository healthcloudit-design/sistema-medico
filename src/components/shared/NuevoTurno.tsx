...
interface Props {
  ...
  /**
   * Si se pasa (caso: un profesional creando su propio turno desde su dashboard),
   * se salta el paso de "elegir profesional" y se usa este directamente. El turno
   * se crea igual que desde recepción — vía la RPC reservar_turno — sin pasar por
   * el flujo de pago/seña de Mercado Pago (ese flujo solo aplica a la reserva
   * pública desde el link de agenda).
   */
  lockedProfessional?: { id: string; full_name: string; specialty?: string } | null
}
...
export function NuevoTurno({ organizationId, initialPatient, lockedProfessional }: Props) {
  const STEPS: Step[] = lockedProfessional
    ? ['servicio', 'fecha', 'paciente', 'confirmar']
    : ['profesional', 'servicio', 'fecha', 'paciente', 'confirmar']

  const [step, setStep]                   = useState<Step>(STEPS[0])
  const [professionals, setProfessionals] = useState<Professional[]>([])
  const [services, setServices]           = useState<Service[]>([])
  const [selectedPro, setSelectedPro]     = useState<Professional | null>(lockedProfessional ?? null)
  ...
  }, [organizationId, lockedProfessional])
  ...
  const reset = () => {
    setStep(STEPS[0])
    setSelectedPro(lockedProfessional ?? null); setSelectedSvc(null)
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Paso 1: Profesional (se salta si lockedProfessional) */}
      {step === 'profesional' && (
