              ...
              return (
                <div key={i} className="flex flex-col items-center flex-1">
                  <div className={['w-9 h-9 rounded-full flex items-center justify-center text-xs font-semibold transition-all',
                    isDone ? 'bg-sky-600 text-white' : isActive ? 'bg-sky-100 text-sky-700 ring-2 ring-sky-500' : 'bg-gray-100 text-gray-400'].join(' ')}>
                    {isDone ? <CheckCircle className="w-4 h-4" /> : <Icon className="w-4 h-4" />}
                  </div>
                  <span className={['text-xs mt-1 hidden sm:block', isActive ? 'text-sky-700 font-medium' : 'text-gray-400'].join(' ')}>
                    {step.label}
                  </span>
                </div>
              )
